...
<section class="w-full mt-10 max-w-sm p-6 m-auto bg-gray-100 rounded-md shadow-md">
    ...
    <form action="{{route('setting-profile.updateImage')}}" enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
        <input type="hidden" name="imageUpdate" id="imageUpdate">
        <div>
            <label for="image" class="mt-2">
                Image
            </label>
            <input type="file" id="image" name="image" class="px-4 py-2 mt-2 border rounded-md">
            @error('image')
            <span class="text-red-400">
                <strong>{{$message}}</strong>
            </span>
